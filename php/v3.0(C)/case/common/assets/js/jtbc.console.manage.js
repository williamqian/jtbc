jtbc.console.manage = {
  ...
  initList: function()
  {
    ...
    managerObj.find('.searchbox').find('input.search').on('click', function(){
      ...
      var url = tthis.para['fileurl'] + thisObj.parent().attr('action') + '&keyword=' + encodeURIComponent(keyword);
      tthis.parent.loadMainURL(url);
    });
    managerObj.find('icon.category').on('click', function(){
      var thisObj = $(this);
      if (thisObj.attr('loading') != 'true')
      {
        thisObj.attr('loading', 'true');
        var url = tthis.para['fileurl'] + thisObj.attr('url') + '&fid=' + encodeURIComponent(thisObj.attr('fid'));
        $.get(url, function(data){
          ...
          if (dataObj.find('result').attr('status') == '1')
          {
            ...
            pageObj.find('ul.list').find('li:not(.alonetips)').on('click', function(){
              ...
              tthis.parent.loadMainURL(tthis.para['fileurl'] + thisObj.attr('link'));
            });
          };
        });
      };
